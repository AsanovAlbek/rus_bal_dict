            ...
            child: PopScope(
                ...
                child: Column(
                  children: [
                    ...
                    if (widget.word?.audioUrl.trim().isNotEmpty ?? false) BlocProvider(
                      create: (context) => DetailBloc(GetIt.I<DetailRepository>())..add(GetAudioEvent(widget.word?.audioUrl)),
                      child: BlocBuilder<DetailBloc, DetailState>(
                        builder: (context, state) {
                          return switch(state) {
                            DetailStateLoaded(bytes: final bytes) => Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                              IconButton.outlined(onPressed: () => _playMusic(bytes: bytes, speed: _normalSpeed), icon: Icon(_iconData)),
                              IconButton.outlined(onPressed: () => _playMusic(bytes: bytes,speed: _slowSpeed), icon: Row(
                                children: [
                                  const Text('Медленно'),
                                  Icon(_iconData),
                                ],
                    ...
                    const SizedBox(height: 12),
                    Expanded(child: SingleChildScrollView(scrollDirection: Axis.vertical, child: Text("${widget.word?.meaning}")))
                  ],
                )),
          ),
        );
