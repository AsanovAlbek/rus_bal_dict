    ...
    return ValueListenableBuilder(
      ...
      builder: (BuildContext context, Box<WordHiveModel> box, Widget? child) {
        var wordsFromHistory = box.values
            .where((word) =>
                word.userId ==
                (context.read<ProfileCubit>().state.appSettings.userInfo.id ??
                    0))
            .toList();
        ...
        final words =
            wordsFromHistory.map((hiveModel) => hiveModel.toModel()).toList();
            ...
            if (words.isNotEmpty)
              SliverList(
                ...
                return WordListItem(
                  word: words[index],
                  onPressed: (word) =>
                      context.go('/history/word_detail', extra: word),
                );
              }, childCount: words.length))
            else
              const SliverFillRemaining(
                  child: Center(child: Text('Здесь пока ничего нет')))
          ],
        );
      },
    );
